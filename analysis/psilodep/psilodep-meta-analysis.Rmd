...
The primary meta-analysis on continuous outcomes in the 12 studies included in the main model showed a statistically significant reduction in depression scores with psilocybin treatment as compared to control conditions, with small to moderate between-study heterogeneity.

```{r main continuous forest plot for indesign, include=FALSE}
quartz(type = "pdf", file = file.path(basedir, "analysis/psilodep/paperfigs/continuous_forest_plot_meta.pdf"), width = 14.3, height = 4.76)
...
subgroup_plot
quartz(type = "pdf", file = file.path(basedir, "analysis/psilodep/paperfigs/subgroup_forest_ggplot.pdf"),
       width = 10, height = 8)
print(subgroup_plot)
dev.off()
...
table_subgroup_df <- full_subgroup_summary %>%
  mutate(
    ...
    # `Tau<U+00B2>` = round(tau2, 2),
    `Tau<U+00B2>` = formatC(signif(tau2, 2), format = "fg", digits = 2, flag = "#"),
...
# Build a vector-format table using gridExtra (gt + gtsave rasterizes via webshot2)
library(gridExtra)
...
tg <- tableGrob(
  ...
  theme = ttheme_minimal(
    base_family = "Arial",
    base_size = 35,
    padding = unit(c(20, 20), "mm"),
    core    = list(fg_params = list(hjust = 0, x = 0.02)),
    colhead = list(fg_params = list(hjust = 0, x = 0.02, fontface = "bold"))
  )
)

quartz(type = "pdf", file = file.path(basedir, "analysis/psilodep/paperfigs/subgroup_table.pdf"),
       width = 20.28, height = 19.86)
grid::grid.draw(tg)
dev.off()
```
...
heterogeneity_text <- sprintf(
  "Heterogeneity: I<U+00B2> = %.2f%% [%.2f%%, %.2f%%], <U+03C4><U+00B2> = %.2f [%.2f, %.2f], p = %.3f",
