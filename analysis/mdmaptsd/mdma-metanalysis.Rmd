...
summary(main_results$model.threelevel.che)
...
sg <- subgroupAnalysis(main_results, condition_arm2)
sg
plot(sg, "condition_arm2")
```


The primary meta-analysis on continuous outcomes in the 12 studies included in the main model showed a statistically significant reduction in PTSD scores with MDMA treatment as compared to control conditions, with small to moderate between-study heterogeneity.

```{r main continuous forest plot for indesign, include=FALSE}
png(filename = file.path(basedir, "analysis/mdmaptsd/paperfigs/continuous_forest_plot_meta.png"), res = 315, width = 4500, height = 1500)
...
png(filename = file.path(basedir, "analysis/mdmaptsd/paperfigs/SI_Fig_02.png"), res = 315, width = 2500, height = 1500)
...
png(filename = file.path(basedir, "analysis/mdmaptsd/paperfigs/SI_Fig_03.png"), res = 315, width = 3000, height = 2200)
...
- In the multi-arm study (Goodwin 2022) replace high-dose intervention (25 mg) with medium-dose intervention (10 mg)
...
full_subgroup_summary <- bind_rows(
  ...
  # sensitivity
  extract_subgroup_summary(g10, "Alternate dosing in Goodwin 2022"),
...
desired_order <- c(
  ...
  "Excluding outliers",
  "Fixed Effects model",
  "Alternate dosing in Goodwin 2022",
...
subgroup_plot
ggsave(file.path(basedir, "analysis/mdmaptsd/paperfigs/subgroup_forest_ggplot.png"),
...
gtsave(subgroup_table, filename = file.path(basedir, "analysis/mdmaptsd/paperfigs/subgroup_table.png"), vwidth = 2000, vheight = 1200, zoom = 10) # setting Zoom higher makes image less blurry/clearer
```
...
Hedges' g values did not differ greatly from the main model, and 10/11 of the subgroup and sensitivity analyses produced significant results.
...
png(filename = file.path(basedir, "analysis/mdmaptsd/paperfigs/expanded_forest_plot_meta.png"), res = 315, width = 4500, height = 1500)
...
png(filename = file.path(basedir, "analysis/mdmaptsd/paperfigs/response_forest_plot_meta.png"), res = 315, width = 4500, height = 1500)
...
png(filename = file.path(basedir, "analysis/mdmaptsd/paperfigs/remission_forest_plot_meta.png"), res = 315, width = 4500, height = 1500)
