...
meta::forest(
  main_results$model.threelevel.che,
  sortvar = main_results$model.threelevel.che$data$year,
  layout = "JAMA"
)
```
```{r subgroup analyses}
...
```{r meta-regression dose, message=FALSE, warning=FALSE}
reg <- metaRegression(time_results$model.threelevel.che, ~dose_arm1)
reg
```

```{r plot-meta-regression, message=FALSE, warning=FALSE, results=FALSE}
png(filename = file.path(basedir, "analysis/mdmaptsd/paperfigs/dose_regression.png"), res = 315, width = 3000, height = 2200)
regplot(reg, mod = "dose_arm1", xlab = "Dose (mg)", ylab = "Hedges' g")
dev.off()
```
...
```{r meta-regression time since dose, message=FALSE, warning=FALSE}
reg <- metaRegression(time_results$model.threelevel.che, ~dose_days)
reg
```

```{r plot-meta-regression, message=FALSE, warning=FALSE, results=FALSE}
png(filename = file.path(basedir, "analysis/mdmaptsd/paperfigs/time_since_dosing.png"), res = 315, width = 3000, height = 2200)
regplot(reg, mod = "dose_days", xlab = "time since dosing (days)", ylab = "Hedges' g")
dev.off()
```
