...
data_functional <- data %>%
  filterPoolingData(
    ...
    !(Detect(study, "Ot'alora 2018") & (!is.na(multi_arm1)) & Detect(multi_arm2, "100 mg")),
    !(Detect(study, "Ot'alora 2018") & (Detect(multi_arm1, "100 mg") & !is.na(multi_arm2))),
    Detect(instrument_symptom, "functional impairment") | Detect(instrument_symptom, "psychological functioning"),
    outcome_type == "msd"
  )
...
par(cex.lab  = 1.8,  # axis titles (xlab, ylab)
...
mtext(
  paste0(
    expression(beta), " = ", sprintf("%.2f", slope),
    ", p = ", format.pval(p_mod, digits = 2, eps = 0.001)
  ),
...
```{r functional impairment model}
# Run meta-analysis

functional_results <- runMetaAnalysis(data_functional,
  which.run = c("overall"),
  # Specify statistical parameters
  es.measure = "g", # Hedges' g
  method.tau = "REML",
...
summary(functional_results$model.overall)
...
```{r functional impairment continuous forest plot for indesign, include=FALSE}
png(filename = file.path(basedir, "analysis/mdmaptsd/paperfigs/functional_impairment_mdmaptsd.png"), res = 315, width = 4500, height = 1500)
...
#   label.right = "Favors control",
#   smlab = "SMD")
meta::forest(functional_results$model.overall,
  sortvar = functional_results$model.overall$data$year,
  leftcols = c("studlab", "condition_arm1", "dose_arm1", "n_arm1", "mean_arm1", "sd_arm1", "condition_arm2", "dose_arm2", "n_arm2", "mean_arm2", "sd_arm2"),
  leftlabs = c("Study", "Drug", "Dose", "N", "Mean","SD", "Drug", "Dose", "N", "Mean", "SD"),
  fontsize = 9,
  col.square = "#000080",
