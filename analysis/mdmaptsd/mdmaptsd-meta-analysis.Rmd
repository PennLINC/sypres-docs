...
quartz(type = "pdf", file = file.path(basedir, "analysis/mdmaptsd/paperfigs/fig1_mdmaptsd.pdf"), width = 14.3, height = 4.76)
...
quartz(type = "pdf", file = file.path(basedir, "analysis/mdmaptsd/paperfigs/three_level_regression_forest.pdf"), width = 14.3, height = 4.76)
plot(
  time_results,
  which = "threelevel.che",
...
base <- file.path(basedir, "analysis/mdmaptsd/paperfigs/dosing_sessions_regression")
quartz(type = "pdf", file = paste0(base, ".pdf"), width = 9.51, height = 6.98)
plot_dosing()
dev.off()
quartz(type = "png", file = paste0(base, ".png"), width = 9.51, height = 6.98,
       dpi = 300)  # dpi matters for PNG sharpness
plot_dosing()
dev.off()
```
...
base <- file.path(basedir, "analysis/mdmaptsd/paperfigs/cumulative_dose")
quartz(type = "pdf", file = paste0(base, ".pdf"), width = 9.51, height = 6.98)
plot_cumulative()
dev.off()
quartz(type = "png", file = paste0(base, ".png"), width = 9.51, height = 6.98, dpi = 300)
plot_cumulative()
dev.off()
```
...
quartz(type = "pdf", file = file.path(basedir, "analysis/mdmaptsd/paperfigs/response_forest_plot_meta_mdmaptsd.pdf"), width = 14.3, height = 4.76)
...
quartz(type = "pdf", file = file.path(basedir, "analysis/mdmaptsd/paperfigs/remission_forest_plot_meta_mdmaptsd.pdf"), width = 14.3, height = 4.76)
...
quartz(type = "pdf", file = file.path(basedir, "analysis/mdmaptsd/paperfigs/depression_mdmaptsd.pdf"), width = 14.3, height = 4.76)
...
- Main model, excluding Mithoefer 2011 that had some concerns in risk of bias assessment
...
```{r run individual meta analysis for paper 1, warning=FALSE, message=FALSE}
...
```{r run fixed effect models, warning=FALSE, message=FALSE}
...
Our fixed effects models yielded results that were in line with the random effect models.

```{r run individual meta analysis for paper 2, warning=FALSE, message=FALSE}
# Use metapsyTools' replacement and rerun functions for quickly changing parameters
...
Our model excluding Mithoefer 2011 yielded similar results to the main model.

```{r main medium dose forest plot for indesign, include=FALSE}
png(filename = file.path(basedir, "analysis/mdmaptsd/paperfigs/mediumdose_mdmaptsd.png"), res = 315, width = 4500, height = 1500)
...
```{r main excluding some concerns RoB forest plot for indesign, include=FALSE}
png(filename = file.path(basedir, "analysis/mdmaptsd/paperfigs/robconcerns_mdmaptsd.png"), res = 315, width = 4500, height = 1500)

meta::forest(robconcerns$model.overall,
  sortvar = robconcerns$model.overall$data$year,
  leftcols = c("studlab", "condition_arm1", "dose_arm1", "n_arm1", "mean_arm1", "sd_arm1", "condition_arm2", "dose_arm2", "n_arm2", "mean_arm2", "sd_arm2"),
  leftlabs = c("Study", "Drug", "Dose", "N", "Mean", "SD", "Drug", "Dose", "N", "Mean", "SD"),
  fontsize = 9,
  col.square = "#000080",
...
png(filename = file.path(basedir, "analysis/mdmaptsd/paperfigs/final/SI_Fig_06.png"), res = 315, width = 2500, height = 1500)
...
total_data_robconcerns <-total_n(data_robconcerns)
print(paste("robconcerns n =", total_data_robconcerns))
```
