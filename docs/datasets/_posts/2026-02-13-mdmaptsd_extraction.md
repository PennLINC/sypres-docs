---
title: "MDMA for PTSD Covidence SOP"
excerpt: "MDMA for PTSD Covidence SOP"
layout: single

---

*SOP for full Covidence workflow on MDMAPTSD_Review*
...
+ If you exclude a report and there is also an accompanying correction for it in full-text review, move the correction back to title and abstract screening and exclude there.
...
+ Mark the correct circle for parallel group or crossover design. *Note: Open-label extension studies should be marked as crossover studies here.
...
CROSSOVER TIMEPOINTS
+ If applicable, list the timepoints (Covidence format, i.e., 'Pre-dose 1') that correspond to times after crossover occured.
+ If not applicable (i.e., parallel group study), leave the box blank.
...
TARGET GROUP
+ This box is meant to capture relevant information regarding the target characteristics of the sample, including any specific population (e.g., COVID healthcare workers), and inclusion criteria regarding any clinical diagnoses.
...
DOSE
+ mg of MDMA (not including any supplemental dose)
...
SUPPLEMENTAL DOSE
+ mg of MDMA given as (sometimes optional) supplemental dose
+ separate sessions by semi-colon (i.e., 25 mg; 25 mg; 30 mg would mean that the participant received 25 mg during the first experimental dosing session, 25 mg during the second experimental dosing session, and 30 mg during the third experimental dosing session)

SUPPLEMENTAL DOSE NOTES
+ Here, write any relevant information about the supplemental dose that is provided (e.g., how many hours it was spaced apart with original dose, if it was optional and how many participants declined the optional dose)

TOTAL DOSE
+ mg of MDMA, totaled between regular dose and (sometimes optional) supplemental dose. Even if some participants did not take the supplemental dose, in this box include the *maximum* dose that a participant could have received for each session if they took the supplemental doses.
+ separate sessions by semi-colon (i.e., 25 mg; 25 mg; 30 mg would mean that the participant received 25 mg during the first experimental dosing session, 25 mg during the second experimental dosing session, and 30 mg during the third experimental dosing session)
...
+ Using the check boxes, select only the amount outcomes in the study that you would like to analyze later
+ Use 'Continuous Outcome #' for continuous outcomes, 'Dichotomous Outcome #' for dichotomous outcomes, 'Baseline Outcomes" for baseline outcomes that are reported in a different format than the rest of the timepoints (see below), "Collapsed Outcome #" for continuous outcomes that are collapsed across both groups AND/OR after an open-label time period, and "Collapsed Outcome # - Dichotomous" for dichotomous outcomes that are collapsed across both froups AND/OR after an open-label time period.
+ In the “describe outcomes” section, underneath the large bold column heading where it prompts ‘Add reported name…’, write the scale acronym.
  ...
  + For response and remission outcomes, be sure to specify the acronym for that scale used to define response and remission (i.e., "BDI Response").
     + Be sure to also specify exactly how response/remission is defined in the corresponding methods box.
     + Some papers provide not only n/N for response and remission, but also present the adjusted response and remission data (i.e., odds ratio adjusted for baseline characteristics, etc.). In these cases, use one of the additional response/remission boxes, and in the 'Add a reported name' description write again what scale was used, adding '(adjusted)' at the end of that same acronym that should be in the corresponding Response 1, Response 2 outcomes (i.e., 'HAM-A Response (adjusted)'). You can later edit the results table such that the adjusted response or remission outcome is reported in the proper format.
 ...
 *ADDING THE SAME OUTCOME BUT AS ENDPOINT OR CHANGE FROM BASELINE*
+ In some cases, the data might be reported as both an endpoint score AND a change from baseline score (either both are reported in the paper, or one or the other is obtained by other means such as contacting authors). Since Covidence does not permit you to enter in the outcome results in two different formats, you will create a new outcome to add either the endpoint or change from baseline additional results.
+ Select "Continuous Outcome #", and write the outcome as "[*Outcome Name*]_endpoint" or "[*Outcome Name*]_change" (i.e., MADRS_endpoint)
+ IMPORTANT: Make sure you add this [*Outcome Name*]_endpoint to relevant sections in the Methods tab:
  + Adjusted Outcomes or Unadjusted Outcomes
  + Change from baseline outcomes or Endpoint outcomes
  + If the newly added outcome is the primary outcome you want to use, add it to 'Chosen Primary Outcome Measure' (separate with a semi-colon, i.e., 'MADRS; MADRS_endpoint')
